...
export function ApplyForm({
  ...
  const status = useMemo(() => {
    ...
    if (existingApplication.isApproved) return "approved"
    if (existingApplication.isActive) return "pending"
    return "rejected"
  }, [existingApplication])
    ...
    <form onSubmit={handleSubmit} className="space-y-8 rounded-3xl border border-[#e5e7eb] bg-white p-8 shadow-sm">
          ...
          {services.map((service) => (
            ...
            >
              <input
                ...
                onChange={() =>
                  toggleSelection(selectedServices, service.id, setSelectedServices)
                }
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-t border-[#f3f4f6] pt-6">
        <p className="max-w-2xl text-sm text-[#6b7280]">
          Al enviar la postulacion vamos a crear tu perfil tecnico en estado pendiente para que
