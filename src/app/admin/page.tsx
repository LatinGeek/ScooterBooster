...
import { Users, Wrench, CalendarDays, DollarSign, Star, Clock, ArrowRight } from "lucide-react"
...
export default async function AdminOverviewPage() {
  ...
  const kpis = [
    { label: "Usuarios registrados", value: totalUsers.toString(), icon: Users, color: "text-[#1d4ed8]", bg: "bg-blue-50" },
    { label: "Técnicos aprobados", value: approvedTechs.toString(), icon: Wrench, color: "text-[#059669]", bg: "bg-[#d1fae5]" },
    { label: "Técnicos pendientes", value: pendingTechs.toString(), icon: Clock, color: "text-amber-700", bg: "bg-amber-50", alert: pendingTechs > 0 },
    { label: "Reservas completadas", value: completedBookings.toString(), icon: CalendarDays, color: "text-[#1d4ed8]", bg: "bg-blue-50" },
    { label: "GMV total", value: formatPrice(totalGMV), icon: DollarSign, color: "text-[#059669]", bg: "bg-[#d1fae5]" },
    { label: "Comisiones cobradas", value: formatPrice(platformRevenue), icon: DollarSign, color: "text-[#d97706]", bg: "bg-amber-50" },
    { label: "Reservas activas", value: pendingBookings.toString(), icon: CalendarDays, color: "text-[#1d4ed8]", bg: "bg-blue-50" },
    { label: "Reseñas totales", value: reviewsSnap.size.toString(), icon: Star, color: "text-[#d97706]", bg: "bg-amber-50" },
  ]
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {kpis.map((kpi) => (
          <div key={kpi.label} className={`rounded-2xl border bg-white p-5 shadow-sm ${kpi.alert ? "border-amber-200" : "border-[#e5e7eb]"}`}>
